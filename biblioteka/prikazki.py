...
import opisvane
from opisvane import zaglavie, prn, dictOrder, dict_lower, make_dict_attr, make_dict_trans, str2list, fnmatch_list, cyr2lat, ntfs_fname_filter
...
def fname_filter( f): return ntfs_fname_filter( f, context= None, repl= '.')
...
class nskaPrikazka:
    re_ia = '([иа])'
    re_nsk = '^([А-Яа-я][а-я]+(н|[сзц]к))'  #полска киргизка гръцка
    re_prik= 'Приказк'
...
class info( opisvane.info):
    ...
    def opts():
        ...
        optz.text( 'sykr_vhod',     help= 'чете файл със съкращения (списъци имена или от грамофонче)', **gg)
        optz.text( 'sykr2zapis',    help= 'прави файл със съкращения като списък', **gg)
        ...
        optz.text( 'tags_app',  type= 'choice', choices= list(mp3tag_apps),
                                    help= 'инструмент слагащ mp3-етикети - ако изобщо', **gg)
        ...
        optz.text( 'url_koren',  default= '/',  help= 'абс.адрес на общия корен', **gg)

        optz.text( 'html_index',    help= 'прави отделни html-страници по папки', **gg)
        optz.text( 'py_index',      help= 'прави отделни данни=списък-новости-избор по папки', **gg)
...
#       optz.append( 'samo_vnos',   default=[], help= 'папка-внос от/обработена другаде - показва се само в списъка', **gg)
        ...
        optz.text( 'podredba',              help= 'подрежда по изброените полета [%default]',
            default= 'humor,ime_sglobeno,pored', **gg)
        ...
        gg = optz.grouparg( 'описи-обобщаващи')
        optz.bool( 'opis_bez_otkoga',    help= 'не слага липсващо откога', **gg)
        optz.bool( 'opis_bez_kachestvo', help= 'не слага липсващо качество', **gg)
        optz.bool( 'opis_bez_samopopylva_ot_fname', help= 'не се досеща излишно', **gg)
        optz.bool( 'opis_avtor_predi_etiketi', help= 'име,автор,етикети,..', **gg)
        optz.bool( 'mnogo_niva',        help= 'позволява много нива и ползва най-вътрешното', **gg)
        optz.bool( 'opis_obedini',      help= 'сглобява обединен опис от всяко поддърво', **gg)
    ...
    def zaredi_danni( klas, args):
        klas.abbr = Abbr2()
        if klas.options.sykr_vhod:
            klas.abbr.cheti_eutf( klas.options.sykr_vhod)
        return super().zaredi_danni( args)

    vse_str_orgs = set()

    def zapis( az, **kargs):
        return super().zapis( opis_avtor_predi_etiketi= az.options.opis_avtor_predi_etiketi, **kargs)
    def samopopylva_ot_fname( az):
        if az.options.opis_bez_samopopylva_ot_fname: return
        super().samopopylva_ot_fname()
    def samopopylva_etiketi( az):
        if az.etiketi.koren: return
        az._samopopylva_etiketi()

    def opravi_vid( az, etiketi):
        _vidove = az.stoinosti.portret, az.stoinosti.prochit, az.stoinosti.teatyr
        if 0*'към вид':
            vid = etiketi.vid or []
            if vid:
    ...
    def _samopopylva_etiketi( az):
        ...
        if az.etiketi.kachestvo or not az.options.opis_bez_kachestvo:
        ...
        if not az.options.opis_bez_otkoga:
    ...
    def opis2hora2dejnosti( az):
        ...
        fname = az.origfname.strip('/').split('/')[-1]
        ...
        for e in [ az.etiketi ] + list( az.prevodi.values()):
            ...
            if isinstance( e.uchastnici, list):
                ...
                r = dejnosti2hora.opis2dejnosti( e.uchastnici, e, fname=fname)
            ...
            for hora in dejnosti2hora.values():
                ...
                for h in hora:
                    ...
                    abbr.dobavi( h, e.ime, fname=fname)
                if hh: hora[:] = hh
            ...
            if az.options2( 'avtor2dejnosti') and e.avtor:
                ...
                ost = [ dejnosti2hora.opis2dejnosti( a, e, dejnost_podrazbirane, fname=fname)
                        for a in aa ]
            ...
            for s in e.sydyrzhanie or ():
                ...
                if s:
                    ...
                    if r.get('avtor'):
                        ...
                        for a in r.avtor:
                            opis = u.opis2dejnosti( a, e, u.posledna_dejnost or dejnost_podrazbirane, fname=fname)
                        #az.uchastnici_vse.dobavi( u, bezplus= True)
                        uchastnici_syd.dobavi( u, bezplus= True)
                l_uchastnici_syd.append( (r,u))
            #TODO check ... uchastnici_syd  ? l_uchastnici_syd ? az.uchastnici_vse ??????
            az.uchastnici_vse.dobavi( uchastnici_syd, bezplus= True)
            if 1 or l_uchastnici_syd:
                #if e is not az.etiketi:
                e._sydyrzhanie_uchastnici_razglobeno = l_uchastnici_syd
    ...
    class Uchastnici( make_dict_trans( dictOrder)):
        ...
        def opis2dejnosti( az, opis, element, dejnost_podrazbirane =None, fname=None):
            abbr = az.abbr
            ...
            for b in opis.split(';'):
                ...
                while bb:
                    ...
                    hora = abbr.dobavi( kalpak, element.ime, fname) #element.fname??
    ...
    def setup_elementi( az):
        ...
        if not az.prevodi and len( az.soundfiles) == 1:
            ...
            for v in az.soundfiles:
                ...
                v.ime_bez_grupa_i_nomer = re.sub( r'^\d+\.', '', v.ime_bez_grupa).strip()
                p._fname = v.name

    def __repr__( me):
        return me.__class__.__name__+'::'+me.fname
    vynshni = {}
    def setup( az):
        rfname = az.origfname
        rvse_prefix = info.vse_prefix_orig
        az.rname = rfname[ len( rvse_prefix):].strip('/')
        fniva = az.fname.rstrip('/').split('/')
        az.vnos = False

        for d in az.options.dirs:
            dr = d.rstrip('/')+'/'
            if az.fname.startswith( dr):
                az.rname = az.fname[ len(dr):]
                break
        else:
        ...
        rurl = az.rname.split('/')
        if az.vnos:
            assert rurl[-1] == fniva[-1]
        else:
            if not az.options.mnogo_niva:
                assert len( rurl) ==1, rurl
        print( *rurl )

        niva = len( rurl)
        for n in range( 1, niva):
            p = tuple( rurl[:n] )
            az.vynshni.setdefault( p, []).append( (niva-n, az))

        rurl = rurl[-1:]        #XXX само едно ниво - най-долното
        if 0:   #работи за приказки/дир-абв/ - но не и за приказки/тмп/дир-абв/
        ...
        def vid4ime( ime, **k):
            ...
            for a in [  #първото от тези
                ...
                #(not az.znak( 'pesen', **k) and not az.znak( 'stihove', **k)) and az.znak( 'prochit', **k) and az.stoinosti.prochit,
                ]:
                    if a:
                        vidi.append( a)
                        break
        ...
        az.v_zaglavieto = [
            ...
            az.etiketi.prochit  and az.stoinosti.prochit,  #има го във .vid4ime
        ]
    ...
    def vidove( az, etiketi, vid ='', uchastnici_vse =(), eradio =False):
        ...
        extendif( vid, etiketi and sorted( v for v in [
                ...
                etiketi.prochit      and az.stoinosti.prochit,
    ...
    def _avtor_s_uchastnici( avtor_s_uchastnici, samo_imena =True, sykr_dejnost =True, edno =False,
        ...
        def dai( h, sykr, bez_roli =True):
            ...
            if sykr or sykr_vse:
                k = info.abbr.dai_kyso( k, original= False) or k
            else:
                k = info.abbr.dai_imepylno( k) or k
            ...
            return k
        def rhh( hh, sykr):
            return [ dai( h, sykr, bez_roli) for h in hh ]
...
def sglobi( ime, avtor='', vid ='', izdanie ='', godina='', nomer =None, html= True, razdeli_avtor =True, prepravi_avtor=True):
    ime = zaglavie( ime)
    r = ime
    if vid:
...
def lipsva_izvadi( t):
    mm = re.split( '(липсва .*)', t, maxsplit= 1)
    if len(mm)<3: return t,''
    start, lipsva, end = [m.strip(' ;') for m in mm]
    t = joinif( ' ; ', [start, end ])
    return t,lipsva
def lipsva_udebeli( t):
    return re.sub( r'(липсва \S+)', bold( r'\1'), t)

def sydyrzhanie2imena( y):
    for a,u in y.get('_sydyrzhanie_uchastnici_razglobeno') or () :
        if not a: yield ''
        else:
            kk = dict(a)
            if kk.get('avtor'):
                x = info._avtor_s_uchastnici( u, samo_imena= False, bez_roli =False, razdeli_avtor =True )
                kk['avtor'] = x
            yield sglobi( html= False, razdeli_avtor=False, **kk)
...
def href( url, ime):
    #HACK
    if info.options.html_href_papka_s_index and url.endswith('/'):
        url += 'index.html'
...
def html4index( x, as_dict =False):
 ...
 <a href="%(k)s">
  <img src="%(i)s" alt="%(alt)s" hspace=5 align=right vspace=3 style="clear:right" >
    ...
    r= ''
    #r +='<div align=right> '
    r += href( info.options.url_koren.rstrip('/')+'/', 'Грамофонче-записи') + ':'
    #r += NBSP+NBSP
...
#    h.razdeli_s_index = h.razdeli.replace( '/">', '/index.html">')
    ...
    h.url_koren = info.options.url_koren
    #SE optmz
    if x.tip == tipove.prikazki and 'приказк' not in h.title.lower(): h.title += '/приказки'

    if as_dict: return DictAttr( html=r, vars= h)
...
$noh1=1;
$head="
<link rel='image_src' href='%(url_koren)s/gramofonche2.jpg'>
<link rel='stylesheet' type='text/css' href='%(url_koren)s/spisyk.css'>
";
...
def html4novi2li( items, novi_prez_dni, html4novi =html4novi):
...
def html_novi( items, options, html4novi ):
    ...
    r+= [ '<li> '+ red for red in html4novi2li( nz, options.novi_prez_dni, html4novi= html4novi) ]
    r+= [ '</ul> <hr width=50%>']
    return '\n'.join(r), nz
...
def tags4mp3( x, options):
    ...
    image = ikoni[0].replace( IKONA, MP3IKONA) if ikoni else None
    ...
    izdanie = x.etiketi.izdanie
    for v in x.soundfiles:
        avtor = x.sykrati_avtori( x.avtor_s_uchastnici( v, edno= True, samo_imena= False))
...
def tags4mp3info( d):
    ...
    return [ 'mp3info', strnoq('$ARGSpred'),
                '-t', d.title,
                '-l', d.album,
                '-a', d.artist,
                '-y', d.godina,
                ...
                '-g', d.genre,
                '-c', d.comment,
                strnoq('$ARGSsled'), '%(file)s'
              ]

def tags4meyed3( d):
    #id3-2
    #old_eyeD3 -a artist -t title -A album -Y year -G genre -p publisher
    ...
    # -F delim   default :
    '''new eyeD3
             [-a artistSTRING] [-A albumSTRING] [-b albumartistSTRING] [-t titleSTRING]
             [-n trackNUM] [-N totalNUM] [-d diskNUM] [-D disktotalNUM] [-G GENRE] [-p publisherSTRING]
             [-Y yearNUM] [-c commentSTRING]
    ...
    cargs = [ 'meyeD3', strnoq('$ARGSpred'),
                '--v12', '--encv1=cp1251',
                '--to-v2.3', '--encoding=utf16',    #XXX що utf16? а не 8?  можеби щото:  utf16=ucs2+bom:v2.2+ ; utf16-be: v2.4 ; utf8: v2.4
                '--remove-all',
            ]
    for atr,opt in opts.items():
        v = d.get(atr)
        if v: cargs += [ opt, v ]
    if d.get('image'): cargs += [ '--add-image', '%(image)s:FRONT_COVER' ]

    cargs += [ strnoq('$ARGSsled'), '%(file)s' ]
    return cargs
...
def tags4mstagger( d):
    ...
    cargs = [ 'mstagger', '%(file)s' , strnoq('$ARGSpred'),
                #'--v12', '--enc=cp1251',
                ]
    for atr,opt in opts.items():
        v = d.get(atr)
        if v: cargs += [ (opt or atr)+'='+v ]
    if d.get('image'): cargs += [ 'picture=%(image)s' ]
    cargs += [ strnoq('$ARGSsled'), ]
    return cargs


mp3tag_apps = dict( meyed3 = tags4meyed3, mp3info = tags4mp3info,
                mstagger = tags4mstagger,
                )

#from svd_util.py.wither import wither

def tags4mp3_all( items, options):
    a2app = mp3tag_apps[ options.tags_app.lower() ]
    o = options
    ...
    for x in items:
        ...
        for args,v in tags4mp3( x, o):
            ...
            r = ' '.join( ['$GO'] + [ a if isinstance( a, strnoq) else '"'+a+'"'  for a in cargs] )
...
class Abbr2( AbbrDejnosti):
    def dobavi( az, tt, xime, fname):
        return AbbrDejnosti.dobavi( az, tt, xime, fname, propusni_takiva= nskaPrikazka.syvpada )
    dejnosti1 = '''
    ...
    dejnosti2 = '''
        ...
        музикално_оформление  .м.оф .муз.оф .м.о  .мо    музикална-картина музикална-среда музикално-оформление муз.оф*ормление муз-оф*ормление музикален-редактор
        звуково_оформление    .зв.оф .зв .з .з.оф*ормление  .зв.еф*екти .з.е*фекти .зе звук звукова-картина звукова-среда звуково-оформление зв.оф*ормление звукови-еф*екти ефекти
        ...
        рис*унка        .рис
        худ*ожник       .худ
        художествено_оформление художествено-оформление .худ.оф .х.оф
        ...
        for vid,sorter in [ [ 'съкращение', None],  [ 'изпълнение', lambda kv: sorted(kv[1])[0] ]]:
            ...
            for a, xime in sorted( abbr.tyrseni.items(), key= sorter ):
                if not abbr.dai_imepylno(a):
                    r.append( ('#? %-20s ' % a) + '; '.join( sorted( ime+': '+fname for ime,fname in xime)) )
...
def podredi( x, as_dict =False):
    ...
    for i in x.options.podredba:
        ...
        r[i] = v
    if not as_dict: r = tuple( r.values())
    else: r = dict(r)
    return r
...
общо: %(n)sброя / %(size)sMb / %(time_h)sчаса ''' % dict( total, time_h= total.time//60)
    ...
    if options.html_novi:
        ...
        rt.insert( 0 if options.obshto_otgore else len(rt), rnovo)
    if options.html_novi_vse:
        save_if_diff( options.html_novi_vse,
            ' <br>\n'.join( html4novi2li( novi, novi_prez_dni=1, html4novi= html4novi)),
...
def items2x( x):
    if x.etiketi.otdelni:
        for v in x.soundfiles: yield (v,x)
    else:
        yield (x,x)
...
def vse( items, options):
    ...
    if options.opis_obedini:
        vynshni_sysedni = dict( (k,vv) for k,vv in info.vynshni.items()
                                if 1 in dict( vv)) #не твърде външни
        za_obedinenie = {}  #външна папка без опис: списък подпапки с описи
        for k,vv in vynshni_sysedni.items():
            if k[:-1] in vynshni_sysedni: continue #има по-външно
            #print( 1111, k, *vv)
            k = '/'.join( k)
            vv = [ v for n,v in vv]
            print( 1111, k, *vv)
            #za_obedinenie[ k ] = vv
            #obedini_opisi( k, vv) TODO
            ''' външен: всичко без: откъде:, издание: повреда: срез: ориг_*: и без празните
            вътрешни:
                опис: ../../opis или май /цял/път
                име: автор: откъде: издание: ориг_*:
                повреда: срез: ..
                '''
        #pprint.pprint( za_obedinenie)
    ...
    if options.py_index:
        for x in items:
            if x.vnos:
                ...
                index = html4index( x, as_dict=True)
                data4papka = DictAttr(
                    total   = calc_total( [x], options),
                    html4novi = list( html4novi( calc_novi( list(items2x(x))))),
                    izbor   = bool( not x.vnos and (x.etiketi.preporyka or x.etiketi.izbor )),
                    index   = index.html,
                    vars    = index.vars,
                    spisyk  = html4spisyk( x),
                    podredba = podredi( x, as_dict=True),
                )
                ofname = join( x.fname, options.py_index)
...
def izdania_spisyk( items, options):
    izd = {}
    def a2list(x):
        return x.split() if isinstance( x, str) else ( [ str(x) ] if isinstance( x, int) else x)
    re_izd = re.compile( r'(\D*)(\d*)(.*)')
    neizv = 0
    izd2neizv = {}
    for az in items:
        if az.vnos: continue
        ...
        ii = [ ]
        if 1:#len(az.soundfiles)>1:
            for v in az.soundfiles:
                if re.search( r'(страна|част) *\d+$', v.ime, re.IGNORECASE): continue
                appendif( shapka.parcheta, v.ime )
                ii+= [ DictAttr(
                            izdanie = v._izdania or shapka.izdanie,
                            godina  = v.godina or shapka.godina,
                            ime     = [ v.ime ],
                            avtor   = az.avtor_s_uchastnici( v, samo_imena= False) or '',
                            vid     = v.vid or shapka.vid,
                            fname   = [ v.relname ],
                            absurl  = [ join( az.absurl, v.name) ],
                            e_shapka= False,
                            )]
        if len( ii)>1 or not ii or shapka.ime != ii[0].ime:
            ii.insert(0, shapka)

        spisyci = 'ime avtor godina fname absurl vid'.split()
        for d in ii:
            xizdanie = d.izdanie
            ...
            for i in xizdanie:
                ...
                for k in spisyci:
                    extendif( di[k],  d[k])
                di.elementi.append( d)

    r = []
    for i in sorted( izd.values(),
                key= lambda i: (i.izdatel, i.nositel, i.samonomer, i.prednomer, i.slednomer) ):
        for a,v in i.items():
            if isinstance( v,set):
                i[a] = sorted( v)
        ...
        o.samonomer = ''
        if i.samonomer>0:
            o.samonomer = str(i.samonomer)
            if i.slednomer.startswith(','):
                o.samonomer += ','+ o.samonomer[ : len(o.samonomer)-len(i.slednomer)+1] + i.slednomer[1:]

        shapka = [ d for d in i.elementi if d.e_shapka]
        if shapka: i = shapka[0]
        o.ime = i.ime
        o.avtor = ', '.join( [ razdeli_kamila2(a) for a in i.avtor])
        #HACK -> aa a, реж. : бб б, ...
        o.avtor = o.avtor.replace( ':, ', ' ') #dejnost
        o.avtor = o.avtor.replace( '  ', ' ')

        o.godina= i.godina
        o.vid   = ' '.join( sorted( i.vid)) or ''
        o.fname = i.fname
        o.url   = i.absurl
        r.append( o)

    red = 'izdatel nositel nomer samonomer ime avtor godina vid url'.split()    #fname
    assert set( red ).issubset( r[0]), set( red ).difference( r[0])
    if 1: #options.izdania_spisyk.endswith( '.csv'):
        ...
        url_pfx=''
        for o in r:
            o.url = [ url_pfx + i for i in o.url ]
            cw.writerow( [ v if isinstance( v, str) else '\n'.join( v)
                            for v in ( o[k] for k in red)
                            ])
        rr = f.getvalue()
