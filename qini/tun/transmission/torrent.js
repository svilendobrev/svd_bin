...
Torrent.prototype =
{
	...
	initialize: function( transferListParent, fileListParent, controller, data) {
		...
		//svd
		var ee = document.createElement( 'div' );	//svd
		// Create the 'name' <div>
		var e = document.createElement( 'div' );
		e.className = 'torrent_name';
		e.style.width = "60%";		//svd
		//top_e.appendChild( e );
		ee.appendChild( e );		//svd
	...
	getPercentDoneStr: function() {
		//svd: no decimals plz
		return Math.floor(100*this.getPercentDone()); 
	},
	...
	getErrorMessage: function()
	{
		if( this._error  == Torrent._ErrTrackerWarning )
			return 'Tracker warning: ' + this._error_string;
		if( this._error  == Torrent._ErrTrackerError )
			return 'Tracker error: ' + this._error_string;
	...
	getPeerDetails: function()
	{
		...
		switch( st )
		{
			...
			case Torrent._StatusDownloading:
				...
				*/ 
				c = 'DL: ';
				c += !this._download_speed ? '0' : ('<b>'+Math.formatBytes(this._download_speed)+'</b>/s');
				c += ' UL: ';
				c += !this._upload_speed ? '0' : ('<b>'+Math.formatBytes(this._upload_speed)+'</b>/s');
				break;
			...
			case Torrent._StatusSeeding:
				...
				c += ' peers - UL: ';
				*/
				c = !this._upload_speed ? '' : ('UL: <b>'+Math.formatBytes(this._upload_speed)+'</b>/s');
				break;
	...
	refreshHTML: function() {
		...
		else if( notDone )
		{
			...
			if( this.isActive( ) )
			{
				...
				if (this._eta < 0 || this._eta >= Torrent._InfiniteTimeRemaining )
					...
					eta += Math.formatSeconds(this._eta);
			}
		...
		else
		{
			...
			//c += ' >> '; //uploaded 
			//c += !this._upload_total ? '0' : Math.formatBytes(this._upload_total);
			c += !this._upload_total ? '' : ' ^'+Math.round(this._upload_ratio*10)/10;
			/*
			c += ' (Ratio ';
