...
Transmission.prototype =
{
	...
	keyDown: function(event)
	{
		...
		var keyCode = event.keyCode;		//svd
		if (keyCode == 40 || keyCode == 34 
		 || keyCode == 38 || keyCode == 33
		 || keyCode == 35 || keyCode == 36
		 ) {
			var i = 0;
			if (sel.length) {
				if (keyCode == 40 || keyCode == 34) { // down arrow, page down
					i = tr.getTorrentIndex(rows, sel[sel.length-1]);
					i += (keyCode == 40) ? 1 : 10;
				} else 
				if (keyCode == 38 || keyCode == 33) { // up arrow, page up
					i = tr.getTorrentIndex(rows, sel[0]);
					i -= (keyCode == 38) ? 1 : 10;
				}
				if (i > rows.length || keyCode==35) i = rows.length-1;	//end
				if (i<0 || keyCode==36) i=0;	//home
			} //eo svd
			...
			i = tr.getTorrentIndex( rows, sel[0]);
			if (i > rows.length-sel.length) i = rows.length-sel.length-1;	//end
	...
	removeSelectedTorrents: function() {
		var torrents = this.getSelectedTorrents( );
		transmission.removeTorrents( torrents );	//svd
		/*
		if( torrents.length )
	...
	removeSelectedTorrentsAndData: function() {
		var torrents = this.getSelectedTorrents( );
		transmission.removeTorrentsAndData( torrents );	//svd
		/*
		if( torrents.length )
