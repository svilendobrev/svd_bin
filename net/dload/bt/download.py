    ...
    try:
        ...
        def finished(finfunc = finfunc, finflag = finflag,
            ...
            if ann[0] is not None:
                ann[0](1)
#### mark it ready
            result = file or info['name']
            result = path.normpath( result)
            link = path.join( 'ready', path.basename( result) )
            if not path.exists( link):
                import os
                make( 'ready', True )   #XXX in curdir...
                os.symlink( path.abspath( result), link)
####
            finfunc()
        rm = [None]
