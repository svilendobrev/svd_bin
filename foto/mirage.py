...
class Base:
    ...
    def __init__(self):
        ...
        self.thumbnail_sizes = ["256", "192", "128", "96", "72", "64", "48", "32"]
        ...
        try:
            opts, args = getopt.getopt(sys.argv[1:], "hRvVsfow:", ["help", "version", "recursive", "verbose", "slideshow", "fullscreen", "onload=", 'writable' ])
        ...
        if has_option('window', 'w'):
            width = conf.getint( *conf._optargz)
        if has_option('window', 'h'):
            height = conf.getint( *conf._optargz)
        if has_option('window', 'toolbar'):
            self.toolbar_show = conf.getboolean( *conf._optargz)
        if has_option('window', 'statusbar'):
            self.statusbar_show = conf.getboolean( *conf._optargz)
        if has_option('window', 'thumbpane'):
            self.thumbpane_show = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'simple-bgcolor'):
            self.simple_bgcolor = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'bgcolor-red'):
        ...
        if has_option('prefs', 'use_last_dir'):
            self.use_last_dir = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'last_dir'):
            self.last_dir = conf.get( *conf._optargz)
        if has_option('prefs', 'fixed_dir'):
            self.fixed_dir = conf.get( *conf._optargz)
        if has_option('prefs', 'open_all'):
            self.open_all_images = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'hidden'):
            self.open_hidden_files = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'open_mode'):
            self.open_mode = conf.getint( *conf._optargz)
        if has_option('prefs', 'last_mode'):
            self.last_mode = conf.getint( *conf._optargz)
        if has_option('prefs', 'listwrap_mode'):
            self.listwrap_mode = conf.getint( *conf._optargz)
        if has_option('prefs', 'slideshow_delay'):
            self.slideshow_delay = conf.getint( *conf._optargz)
        if has_option('prefs', 'slideshow_random'):
            self.slideshow_random = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'zoomquality'):
            self.zoomvalue = conf.getint( *conf._optargz)
        ...
        if has_option('prefs', 'quality_save'):
            self.quality_save = conf.getint( *conf._optargz)
        if has_option('prefs', 'disable_screensaver'):
            self.disable_screensaver = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'slideshow_in_fullscreen'):
            self.slideshow_in_fullscreen = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'preloading_images'):
            self.preloading_images = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'thumbsize'):
            self.thumbnail_size = conf.getint( *conf._optargz)
        if has_option('prefs', 'screenshot_delay'):
            self.screenshot_delay = conf.getint( *conf._optargz)
        if has_option('actions', 'num_actions'):
            num_actions = conf.getint( *conf._optargz)
            self.action_names = []
            ...
            for i in range(num_actions):
                ii = '['+str(i)+']'
                items = 'names commands shortcuts batch'.split()
                if all( conf.has_option( 'actions', t + ii) for t in items):
                    for t in items:
                        confget = conf.getboolean if t=='batch' else conf.get
                        getattr( self, 'action_'+t).append( confget( 'actions', t+ii))
        if has_option('prefs', 'savemode'):
            self.savemode = conf.getint( *conf._optargz)
        if has_option('prefs', 'start_in_fullscreen'):
            self.start_in_fullscreen = conf.getboolean( *conf._optargz)
        if has_option('prefs', 'confirm_delete'):
            self.confirm_delete = conf.getboolean( *conf._optargz)
        self.recentfiles = []
        if has_option('recent', 'num_recent'):
            num_recent = conf.getint( *conf._optargz)
            for i in range(num_recent):
                self.recentfiles.append('')
                if has_option('recent', 'urls[' + str(i) + ',0]'):
                    self.recentfiles[i] = conf.get( *conf._optargz)
    ...
    def thumbpane_set_image(self, image_name, imgnum, force_update=False):
        if self.thumbpane_show:
            if not self.thumbnail_loaded[imgnum] or force_update:
                ...
                if pix:
                    if self.thumbnail_size != THUMBSIZE:
    ...
    def thumbpane_get_pixbuf(self, thumb_url, image_url, force_generation):
        ...
        try:
            ...
            pix = gtk.gdk.pixbuf_new_from_file(imgfile) .apply_embedded_orientation()
            pix, image_width, image_height = self.get_pixbuf_of_size(pix, THUMBSIZE, gtk.gdk.INTERP_TILES)
            st = os.stat(imgfile)
            file_mtime = str(st[stat.ST_MTIME])
    ...
    def thumbpane_get_size(self):
        return int(self.thumbnail_size * 1.02)
    ...
    def print_usage(self):
        ...
        print "  -w, --writable       " + _("Enable saving images.. (default is read-only as EXIF is lost)")


    def _set_timer4slideshow( self):
        self.timer_delay = gobject.timeout_add( int(self.curr_slideshow_delay*1000),
                                self.goto_random_image if self.curr_slideshow_random else self.goto_next_image,
                                "ss")
    def delay_changed(self, action):
        self.curr_slideshow_delay = self.ss_delayspin.get_value()
        if self.slideshow_mode:
            gobject.source_remove(self.timer_delay)
            self._set_timer4slideshow()
        self.window.set_focus(self.layout)
    ...
    def save_image_now(self, dest_name, filetype):
        try:
            assert self.writable, 'readonly, use -w'
            self.change_cursor(gtk.gdk.Cursor(gtk.gdk.WATCH))
            while gtk.events_pending():
                gtk.main_iteration()
            if filetype == None:
            ...
            error_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING, gtk.BUTTONS_CLOSE, _('Unable to save %(dest_name)s : %(e)s') % locals())
            error_dialog.set_title(_("Save"))
            error_dialog.run()
            error_dialog.destroy()
        self.change_cursor(None)
    ...
    def update_preview(self, file_chooser, preview):
        ...
        if pixbuf:
            ...
            pixbuf = gtk.gdk.Pixbuf(gtk.gdk.COLORSPACE_RGB, 1, 8, THUMBSIZE, THUMBSIZE)
            ...
            if response  == gtk.RESPONSE_YES:
                try:
                    if 0:
                        ...
                        os.rename(self.currimg_name, self.currimg_name+'_del')
                    ...
                    if 1:
                        try:
                            self.image_list.remove( self.currimg_name )
                    ...
                    error_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING, gtk.BUTTONS_OK, _('Unable to delete %s : %s') % (self.currimg_name, e))
    ...
    load_when_idle = preload_when_idle = preload_when_idle2 = None
    def load_new_image_stop_now(self):
        for s in 'load_when_idle preload_when_idle preload_when_idle2'.split():
            v = getattr( self, s)
            if v is not None:
    ...
    _img_items = ''' name width height pixbuf pixbuf_original zoomratio is_animation '''.split()
    def check_preloadimg_prev_for_existing(self, prev_index, reset_preloadimg_prev_in_list):
        ...
        if prev_index != self.preloadimg_prev_in_list and prev_index != -1:
            ...
            if prev_index == self.loaded_img_in_list and not self.image_modified and not self.image_zoomed:
                ...
                for i in self._img_items: setattr( self, 'preloadimg_prev_'+i, getattr( self, 'currimg_'+i))
            elif prev_index == self.preloadimg_next_in_list:
                self.preloadimg_prev_in_list = self.preloadimg_next_in_list
                for i in self._img_items: setattr( self, 'preloadimg_prev_'+i, getattr( self, 'preloadimg_next_'+i))
            else:
    ...
    def check_preloadimg_next_for_existing(self, next_index, reset_preloadimg_next_in_list):
        ...
        if next_index != self.preloadimg_next_in_list and next_index != -1:
            ...
            if next_index == self.loaded_img_in_list and not self.image_modified and not self.image_zoomed:
                ...
                for i in self._img_items: setattr( self, 'preloadimg_next_'+i, getattr( self, 'currimg_'+i))
            elif next_index == self.preloadimg_prev_in_list:
                self.preloadimg_next_in_list = self.preloadimg_prev_in_list
                for i in self._img_items: setattr( self, 'preloadimg_next_'+i, getattr( self, 'preloadimg_prev_'+i))
            else:
    ...
    def check_currimg_for_existing(self):
        ...
        if self.curr_img_in_list == self.loaded_img_in_list:
            ...
            if self.curr_img_in_list == self.preloadimg_prev_in_list:
                src = 'preloadimg_prev_'
                used_prev = True
            elif self.curr_img_in_list == self.preloadimg_next_in_list:
                src = 'preloadimg_next'
                used_next = True
            else: return used_prev, used_next

            for i in self._img_items: setattr( self, 'currimg_'+i, getattr( self, src+i))
            if self.verbose and self.currimg_name != "":
                print _("Loading: %s") % self.currimg_name
            self.put_zoom_image_to_window(True)
            self.set_image_sensitivities( not self.currimg_is_animation)
        ...
        if used_prev or used_next:
            ...
            if animtest.is_static_image() or (use_current_pixbuf_original and not self.currimg_is_animation):
                ...
                if not use_current_pixbuf_original:
                    self.currimg_pixbuf_original = animtest.get_static_image().apply_embedded_orientation()
            ...
            if self.preloading_images and len(self.image_list) > 1:
                if not use_existing_image:
                    ...
                    if pre_animtest.is_static_image():
                        self.preloadimg_next_is_animation = False
                        self.preloadimg_next_pixbuf_original = pre_animtest.get_static_image().apply_embedded_orientation()
                    else:
        ...
        try:
            if self.preloading_images and len(self.image_list) > 1:
                if not use_existing_image:
                    ...
                    if pre_animtest.is_static_image():
                        self.preloadimg_prev_is_animation = False
                        self.preloadimg_prev_pixbuf_original = pre_animtest.get_static_image().apply_embedded_orientation()
                    else:
...
if __name__ == "__main__":
    #import glib
    #glib.g_log_set_handler( None, .., lambda: None, None)  pfff no such thing at all
    import sys
    sys_stderr = sys.stderr
    class stderr:
        def write( me, t):
            if 'source_remove' in t: return
            sys_stderr.write(t)
    sys.stderr = stderr()
    try:
        base = Base()
