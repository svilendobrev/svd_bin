                    ...
                    CopyInfo *copy_info ) /* totals info */
{
  ...
  con_out( con_max_x() - strlen(t), con_max_y(), t, cSTATUS2 );
}
...
//if (!dont_size)
  ...
  TF * sel1 = files_list[FLI];
  if ( !a_one )     //find 1st selected
  ...
  bool dont_size = ( a_mode == CM_LINK
                        ||
                     a_mode == CM_MOVE
                        &&
                        device_is_same( sel1->full_name(), target) == 0 );
...
void vfu_erase_files( int a_one )
{
  ...
  VString str;
  fsize_t erase_size = -1; //opt.copy_calc_totals ? vfu_update_sel_size( a_one ) : -1;
  if ( erase_size != -1 )
    {
    str.fi( erase_size );
    str_comma( str );
